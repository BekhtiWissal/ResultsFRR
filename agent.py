...
class Agent(object):
    ...
    def train(self):
        ...
        closest_indices_lists = np.asarray([x[7] for x in data])
        next_closest_indices_lists = np.asarray([x[8] for x in data])
        not_done = (done+1) % 2

        td, _ = self.dqn.training_qnet(coords, state, action, reward, not_done, next_coords, next_state, closest_indices_lists, next_closest_indices_lists)
